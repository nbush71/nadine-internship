import React, { useEffect, useState } from "react";
import EthImage from "../images/ethereum.svg";
import { Link, useParams } from "react-router-dom";
import Skeleton from "react-loading-skeleton";

const ItemDetails = () => {
  const { id: paramsId } = useParams();
  const [author, setAuthor] = useState({
    id: "",
    title: "",
    tag: "",
    description: "",
    nftImage: "",
    nftId: "",
    ownerName: "",
    ownerImage: "",
    ownerId: "",
    creatorName: "",
    creatorId: "",
    creatorImage: "",
    price: "",
    likes: "",
    views: "",
  });
  const [isLoading, setIsLoading] = useState(true);

  async function fetchItems() {
    try {
      const response = await fetch(
        `https://us-central1-nft-cloud-functions.cloudfunctions.net/itemDetails?nftId=${paramsId}`
      );

      const item = await response.json();

      const {
        id,
        title,
        tag,
        description,
        nftImage,
        ownerName,
        ownerImage,
        nftId,
        ownerId,
        creatorName,
        creatorId,
        creatorImage,
        price,
        likes,
        views,
      } = item;

      setAuthor({
        id,
        title,
        tag,
        description,
        nftImage,
        nftId,
        ownerName,
        ownerImage,
        ownerId,
        creatorName,
        creatorId,
        creatorImage,
        price,
        likes,
        views,
      });
    } catch (error) {
      console.error("Error fetching item details:", error);
    } finally {
      setIsLoading(false);
    }
  }

  useEffect(() => {
    setIsLoading(true);
    fetchItems();
    window.scrollTo(0, 0);
  }, [paramsId]);

  return (
    <div id="wrapper">
      <div className="no-bottom no-top" id="content">
        <div id="top"></div>
        <section aria-label="section" className="mt90 sm-mt-0">
          <div className="container">
            <div className="row">
              {isLoading ? (
                <>
                  <div className="col-md-6 text-center">
                    <Skeleton width={500} height={500} borderRadius={20} />
                  </div>
                  <div className="col-md-6">
                    <div className="item_info">
                      <Skeleton width={50} height={20}/>

                      <div className="item_info_counts">
                        <div className="item_info_views">
                          <i className="fa fa-eye"></i>
                          <Skeleton width={50} height={20}/>
                        </div>
                        <div className="item_info_like">
                          <i className="fa fa-heart"></i>
                          <Skeleton width={50} height={20}/>
                        </div>
                      </div>

                      <Skeleton width={300} height={100}/>

                      <div className="d-flex flex-row">
                        <div className="mr40">
                          <Skeleton width={100} height={20}/>
                          <div className="item_author">
                            <div className="author_list_pp">
                              <Link to={`/author/${author.ownerId}`}>
                                <Skeleton width={50} height={50} borderRadius={500}/>
                                <i className="fa fa-check"></i>
                              </Link>
                            </div>
                            <div className="author_list_info">
                              <Skeleton width={100} height={20}/>
                            </div>
                          </div>
                        </div>
                        <div></div>
                      </div>

                      <div className="de_tab tab_simple">
                        <div className="de_tab_content">
                          <h6>Creator</h6>
                          <div className="item_author">
                            <div className="author_list_pp">
                              <Link to={`/author/${author.creatorId}`}>
                                <Skeleton width={50} height={50} borderRadius={500} />
                                <i className="fa fa-check"></i>
                              </Link>
                            </div>
                            <div className="author_list_info">
                              <Skeleton width={100} height={20} />
                            </div>
                          </div>
                        </div>
                        <div className="spacer-40"></div>
                        <h6>Price</h6>
                        <div className="nft-item-price">
                          <img src={EthImage} alt="" />
                          <Skeleton width={50} height={40}/>
                        </div>
                      </div>
                    </div>
                  </div>
                </>
              ) : (
                <>
                  <div className="col-md-6 text-center">
                    <img
                      src={author.nftImage}
                      className="img-fluid img-rounded mb-sm-30 nft-image"
                      alt=""
                    />
                  </div>
                  <div className="col-md-6">
                    <div className="item_info">
                      <h2>{author.title}</h2>

                      <div className="item_info_counts">
                        <div className="item_info_views">
                          <i className="fa fa-eye"></i>
                          {author.views}
                        </div>
                        <div className="item_info_like">
                          <i className="fa fa-heart"></i>
                          {author.likes}
                        </div>
                      </div>

                      <p>{author.description}</p>

                      <div className="d-flex flex-row">
                        <div className="mr40">
                          <h6>{author.ownerName}</h6>
                          <div className="item_author">
                            <div className="author_list_pp">
                              <Link to={`/author/${author.ownerId}`}>
                                <img className="lazy" src={author.ownerImage} alt="" />
                                <i className="fa fa-check"></i>
                              </Link>
                            </div>
                            <div className="author_list_info">
                              <Link to={`/author/${author.ownerId}`}>{author.ownerName}</Link>
                            </div>
                          </div>
                        </div>
                        <div></div>
                      </div>

                      <div className="de_tab tab_simple">
                        <div className="de_tab_content">
                          <h6>Creator</h6>
                          <div className="item_author">
                            <div className="author_list_pp">
                              <Link to={`/author/${author.creatorId}`}>
                                <img className="lazy" src={author.creatorImage} alt="" />
                                <i className="fa fa-check"></i>
                              </Link>
                            </div>
                            <div className="author_list_info">
                              <Link to={`/author/${author.creatorId}`}>{author.creatorName}</Link>
                            </div>
                          </div>
                        </div>
                        <div className="spacer-40"></div>
                        <h6>Price</h6>
                        <div className="nft-item-price">
                          <img src={EthImage} alt="" />
                          <span>{author.price}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </>
              )}
            </div>
          </div>
        </section>
      </div>
    </div>
  );
};

export default ItemDetails;